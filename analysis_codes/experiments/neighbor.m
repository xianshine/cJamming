function [p_n_calc SSE] =  neighbor(omega_max, n,r, display_result,gene)
p_n_calc = zeros(length(n),length(r));
for ii = 1:length(n)
    for jj = 1:length(r)
        p_n_calc(ii,jj) =p_neighbor(n(ii,1),r, omega_max, r(jj,1));
    end
end
p_n_calc = sum(p_n_calc.*(ones(length(n),1)*(r(:,2))'),2)*mean(diff(r(:,1)));
SSE = sum((p_n_calc-n(:,2)).^2);

if display_result
    plot(p_n_calc); hold on
    plot(n(:,2),'.r');hold off
    title([gene,' \omega_{max}=',num2str(omega_max/pi)]);
    xlabel('# of neighbor');
    ylabel('pdf');
    legend('fit','data');
    drawnow
